...
class KubeLibrary(object):
    ...
    def get_rp(self, name_pattern, namespace, label_selector=""):
        ...
        - ``namespace``:
          Namespace to check
        logger.info("herererereeee")
        ret = self.v1.list_namespaced_replicasets(namespace, watch=False, label_selectorr = re.compile(name_pattern)
        replicasets = [item for item in ret.items if r.match(item.metadata.name)]       return replicasets
